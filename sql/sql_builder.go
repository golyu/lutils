...
func (this *SqlBuilder) LikeRight(value string, why string) *SqlBuilder {
	if why == "" {
		panic("no where arguments 'why'")
	}
	if strings.ContainsAny(why, ">=<") {
...
func (this *SqlBuilder) String() (string, error) {
	...
	case 3:
		...
		return "UPDATE " + this.tableName + this.set + this.where, nil
	case 4:
...
//func main() {
//sql := new(SqlBuilder)
//sql.Select("l_user_log").Where("xx", "site_id=").
//	Column("count(1) as num").
//	Where_("infe", "index_id=").Where_("tt", "at_id=").
//	Where_(8, "ua_id=").Where_(0, "sh_id=").
//	Where_(4, "username=").Where_(8, "add_time>=").
//	Where_(1, "add_time<=").Where_(1, "ip=").
//	Limit(int(6)).Offset(int(6 * (6 - 1)))
//fmt.Println(sql)
//sql = new(SqlBuilder)
//sql.Update("l_agent").Set("ssid", "ssid=").Set("logintimeeeee", "login_time=").
//	Where("ooo", "site_id=").Where("username", "username=")
//fmt.Println(sql)
//	fmt.Println(new(SqlBuilder).Insert("l_agent_fc_limit").Value(0, "uid").Value("", "site_id").String())
//	fmt.Println(new(SqlBuilder).Insert("l_agent_fc_limit").Value_(0, "uid").Value_("", "site_id").String())
//
//}
